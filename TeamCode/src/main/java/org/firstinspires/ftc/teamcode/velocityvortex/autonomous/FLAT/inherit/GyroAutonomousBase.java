...
/**
 * Created by Nathan.Smith.19 on 2/6/2017.
 */
@Disabled
public class GyroAutonomousBase extends LinearOpMode {
    ...
    DcMotor                       LEFT_DRIVE_MOTOR,
                                 RIGHT_DRIVE_MOTOR, //Drive train motors
    ...
    protected void beacon(double speed, double time, int rot, GameSide side){
        drive(speed, time, true, true);
        turn(0.1, rot, 0.1);
        sleep(900);
        inputData("c_flash","B:" + COLOR_SENSOR.blue() + "R:" + COLOR_SENSOR.red(), true);
        switch(side){
            case BLUE:
                if(COLOR_SENSOR.blue() > COLOR_SENSOR.red()) press(side);
                else{
                    drive(0.15, 0.75, true, true);
                    press(side);
                }
                break;
            case RED:
                if(COLOR_SENSOR.red() > COLOR_SENSOR.blue()) press(side);
                else{
                    drive(-0.15, 0.75, true, true);
                    press(side);
                }
     ...
     */
    @Deprecated
    private void drive(double speed, double time, int angle, boolean stop){
        ...
        if(stop)stopDriveMotors(true);
    }

    protected void drive(double power, double time, boolean stop, boolean brake){
        LEFT_DRIVE_MOTOR.setPower(power);
        RIGHT_DRIVE_MOTOR.setPower(power);
        time = time * 1000;
        MAIN_RUNTIME.reset();
        while(MAIN_RUNTIME.milliseconds() < time && opModeIsActive()) idle();
        if(stop)stopDriveMotors(brake);
    }
    ...
    protected void findBeaconLine(double speed){
    ...
    /*
    Time input in seconds, then converted to milliseconds.
     */
    protected void shoot(double time, boolean reload){
        time *= 1000;
    ...
    protected void press(GameSide side){
        BEACON_PRESS.setPower(-.5);
        SECOND_RUNTIME.reset();
        if(side == BLUE)
            while((SECOND_RUNTIME.milliseconds() < (2.5 * 1000)) && opModeIsActive() /*&& (BPD_SENSOR.getLightDetected() > DISTANCE_THRESHOLD)*/){idle();}
        else
            while((SECOND_RUNTIME.milliseconds() < (2.5 * 1000)) && opModeIsActive() /*&& (BPD_SENSOR.getLightDetected() > DISTANCE_THRESHOLD)*/){idle();}
     ...
     */
    protected double getSteerForce(double error, double COEFF){
     ...
     */
    protected double getHeadError(double angle){
     ...
     */
    protected void inputData(String IN, String OUT, boolean PUSH){
     ...
     */
    protected void stopDriveMotors(boolean brake){
        if(brake){
            LEFT_DRIVE_MOTOR.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
            RIGHT_DRIVE_MOTOR.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        }else{
            LEFT_DRIVE_MOTOR.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.FLOAT);
            RIGHT_DRIVE_MOTOR.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.FLOAT);
        }
